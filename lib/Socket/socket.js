...
import { addTransactionCapability, aesEncryptCTR, bindWaitForConnectionUpdate, buildPairingQRData, buildEncryptedPairingRequest, buildShortcakeProloguePayload, bytesToCrockford, computePairingHandoffProof, configureSuccessfulPairing, Curve, decodePrimaryEphemeralIdentity, derivePasskeyHandoffKey, derivePairingCodeKey, deriveShortcakeEncryptionKey, deriveShortcakeVerificationCode, generateLoginNode, generateMdTagPrefix, generateRegistrationNode, getCodeFromWSError, getCompanionPlatformId, getErrorCodeFromStreamError, getNextPreKeysNode, makeEventBuffer, makeNoiseHandler, promiseTimeout, signedKeyPair, xmppSignedPreKey } from '../Utils/index.js';
import { assertNodeErrorFree, binaryNodeToString, encodeBinaryNode, getAllBinaryNodeChildren, getBinaryNodeChild, getBinaryNodeChildBuffer, getBinaryNodeChildren, isLidUser, jidDecode, jidEncode, S_WHATSAPP_NET } from '../WABinary/index.js';
import { BinaryInfo } from '../WAM/BinaryInfo.js';
import { USyncQuery, USyncUser } from '../WAUSync/index.js';
import { WebSocketClient } from './Client/index.js';
import { executeWMexQuery } from './mex.js';
export const isConnectionStale = (lastDateRecv, keepAliveIntervalMs, now = Date.now()) => {
    if (!lastDateRecv) {
        return false;
    }
...
export const makeSocket = (config) => {
    ...
    let passkeyLinkingState;
    let passkeyBridgeId;
    let passkeyHandoffKey;
    ...
    const assertOutboundConnectionAlive = () => {
        if (isConnectionStale(lastDateRecv, keepAliveIntervalMs)) {
            ...
            logger.warn({ inactiveForMs, keepAliveIntervalMs }, 'outbound send blocked because socket is open but stale; forcing reconnect');
            void end(error);
            throw error;
        }
    };
    ...
    const sendPasskeyResponse = async ({ credentialId, assertionJson }) => {
    ...
    const handlePasskeyContinuation = async (node) => {
        if (node.attrs.from && node.attrs.from !== S_WHATSAPP_NET) {
        ...
        passkeyLinkingState.encryptionKey = deriveShortcakeEncryptionKey(passkeyLinkingState.keyPair.private, primary.publicKey, passkeyLinkingState.deviceType, passkeyLinkingState.ref);
        const code = deriveShortcakeVerificationCode(passkeyLinkingState.companionNonce, primary.publicKey, primary.nonce);
        ev.emit('passkey.update', {
            status: 'confirmation',
