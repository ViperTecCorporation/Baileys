...
export const downloadHistory = async (msg, options) => {
    ...
    if (!buffer.length) {
        throw new Error(`empty inflated history sync payload mediaKeyBytes=${mediaKeyBytes} fileSha256Bytes=${fileSha256Bytes} fileEncSha256Bytes=${fileEncSha256Bytes} directPathLength=${directPathLength}`);
    }
...
export const processHistoryMessage = (item, logger) => {
    ...
    if (historySyncDiagnosticsEnabled()) {
        logger?.info({
            ...
            syncTypeName: typeof item.syncType === 'number'
...
export const downloadAndProcessHistorySyncNotification = async (msg, options, logger) => {
    let historyMsg;
    const syncTypeName = typeof msg.syncType === 'number'
    ...
    if (msg.initialHistBootstrapInlinePayload) {
        try {
            ...
            if (historySyncDiagnosticsEnabled()) {
                logger?.info({ syncType: msg.syncType, syncTypeName, inlinePayloadBytes: msg.initialHistBootstrapInlinePayload.length, inflatedBytes: inflated.length }, 'inflated inline history sync payload');
            }
