...
import { shouldIncludeReportingSecret } from './reporting-utils.js';
const MIMETYPE_MAP = {
    image: 'image/jpeg',
    video: 'video/mp4',
...
const formatNativeFlowButton = (button) => {
    switch (button.type) {
        case 'url':
            return {
                name: 'cta_url',
...
const generateCarouselMessage = async (carousel, options) => {
    ...
    const recoverImageMetadata = async (imageMessage, image, cardTitle) => {
        ...
        try {
            ...
            imageMessage.jpegThumbnail || (imageMessage.jpegThumbnail = thumb.buffer);
            imageMessage.width || (imageMessage.width = thumb.original.width);
            imageMessage.height || (imageMessage.height = thumb.original.height);
        }
        catch (error) {
    ...
    const carouselCards = await Promise.all(cards.map(async (card, index) => {
        ...
        if (card.image) {
            ...
            if (imageMessage && (!imageMessage.width || !imageMessage.height)) {
                imageMessage.width || (imageMessage.width = 500);
                imageMessage.height || (imageMessage.height = 500);
            }
...
export const generateWAMessageContent = async (message, options) => {
    var _a, _b;
    let m = {};
    const normalizeCarouselCardMedia = async (card) => {
        ...
        const prepared = await prepareWAMessageMedia(mediaMessage, options);
        ...
        if (interactive?.carouselMessage?.cards?.length) {
            ...
            interactive = {
                ...
                carouselMessage: {
                    ...
                    carouselCardType: interactive.carouselMessage.carouselCardType ??
    ...
    if ('buttons' in message && !!message.buttons) {
        const buttonsMessage = {
            buttons: message.buttons.map(b => {
                ...
                if (b.sections) {
                    const buttonText = b.text || b.buttonText?.displayText || b.buttonText || message.buttonText;
                    return {
                        nativeFlowInfo: {
                            name: 'single_select',
        ...
        if ('text' in message) {
            ...
            buttonsMessage.headerType = ButtonType[normalizedHeaderType];
    ...
    if (hasSections || hasProductListInfo) {
        ...
        const listType = requestedListType ??
...
const normalizeInteractiveResponseMessage = (content) => {
    if (!content) {
        return;
    }
...
export const patchMessageForMdIfRequired = (message) => {
    ...
    if (requiresPatch) {
        ...
        if (messageContextInfo) {
            delete messageWithoutContext.messageContextInfo;
        }
        message = {
