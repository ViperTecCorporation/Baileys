...
export const processSyncAction = (syncAction, ev, me, initialSyncOpts, logger, options) => {
    ...
    if (action?.muteAction) {
        ...
        logger?.debug({ operation: syncAction.operation, type, saltBytes }, 'received nct salt sync action');
        if (syncAction.operation === proto.SyncdMutation.SyncdOperation.REMOVE) {
            ...
            logger?.warn({ operation: syncAction.operation, type, saltBytes }, 'nct salt sync action without salt bytes');
        }
    }
    else {
