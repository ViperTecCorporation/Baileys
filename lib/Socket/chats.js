...
import { buildTcTokenFromJid, storeNctSalt } from '../Utils/tc-token-utils.js';
...
export const makeChatsSocket = (config) => {
                ...
                processSyncAction(mutation, ev, authState.creds.me, isInitialSync ? { accountSettings: authState.creds.accountSettings } : undefined, logger, {
                    onNctSalt: async (salt) => {
                        ...
                        try {
                            ...
                            logger.info({ saltBytes, removed: !saltBytes }, 'persisted nct salt from app state');
                        }
                        catch (err) {
                            logger.warn({ saltBytes, removed: !saltBytes, err: err?.message || String(err) }, 'failed to persist nct salt from app state');
                            throw err;
                        }
                    }
                });
    ...
    const profilePictureUrl = async (jid, type = 'preview', timeoutMs) => {
        const pictureNode = { tag: 'picture', attrs: { type, query: 'url' } };
        const content = [pictureNode];
