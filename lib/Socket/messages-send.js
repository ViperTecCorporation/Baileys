...
import { aggregateMessageKeysNotFromMe, assertMediaContent, assertMeId, bindWaitForEvent, decryptMediaRetryData, DEF_MEDIA_HOST, encodeNewsletterMessage, encodeSignedDeviceIdentity, encodeWAMessage, encryptMediaRetryRequest, extractDeviceJids, generateMessageIDV2, generateParticipantHashV2, generateWAMessage, getStatusCodeForMediaRetry, getUrlFromDirectPath, getWAUploadToServer, MessageRetryManager, normalizeMessageContent, parseAndInjectE2ESessions, patchMessageForMdIfRequired, unixTimestampSeconds } from '../Utils/index.js';
import { getUrlInfo } from '../Utils/link-preview.js';
import { makeKeyedMutex, makeMutex } from '../Utils/make-mutex.js';
import { getMessageReportingToken, shouldIncludeReportingToken } from '../Utils/reporting-utils.js';
import { isRetryableStaleConnectionError } from '../Utils/retryable-send.js';
import { buildCsTokenFromStoredSalt, isTcTokenExpired, resolveTcTokenJid, shouldSendNewTcToken, storeTcTokensFromIqResult } from '../Utils/tc-token-utils.js';
import { areJidsSameUser, getBizBinaryNode, getBinaryNodeChild, getBinaryNodeChildren, isHostedLidUser, isHostedPnUser, isJidGroup, isLidUser, isPnUser, jidDecode, jidEncode, jidNormalizedUser, shouldIncludeBizBinaryNode, S_WHATSAPP_NET } from '../WABinary/index.js';
import { USyncQuery, USyncUser } from '../WAUSync/index.js';
import { makeNewsletterSocket } from './newsletter.js';
export const makeMessagesSocket = (config) => {
    const { logger, linkPreviewImageThumbnailWidth, generateHighQualityLinkPreview, options: httpRequestOptions, patchMessageBeforeSending, cachedGroupMetadata, enableRecentMessageCache, maxMsgRetryCount, privacyTokenQueryTimeoutMs } = config;
    ...
    const relayMessage = async (jid, message, options) => {
        let { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList } = options;
        ...
        const shouldForceCarouselDeviceIdentity = !!(message.interactiveMessage?.carouselMessage ||
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroupOrStatus && !isRetryResend) {
                ...
                const viewOnceInner = message.viewOnceMessageV2?.message ||
                    message.viewOnceMessage?.message ||
                    message.viewOnceMessageV2Extension?.message;
                const isViewOnceMsg = !!(viewOnceInner?.imageMessage?.viewOnce ||
            ...
            if (is1on1Send && isLidUser(destinationJid) && !additionalAttributes?.peer_recipient_pn) {
                try {
                    ...
                    if (peerRecipientPn && isPnUser(peerRecipientPn)) {
                        ...
                        logger.debug({ jid: destinationJid, peerRecipientPn: additionalAttributes.peer_recipient_pn }, 'attached peer_recipient_pn for LID 1:1 message');
                    }
                }
                catch (err) {
            ...
            let privacyTokenNodeTag;
            // Resolve destination to LID for tctoken storage — matches Signal session key pattern
            const tcTokenJid = is1on1Send ? await resolveTcTokenJid(destinationJid, getLIDForPN) : destinationJid;
            const peerRecipientPn = is1on1Send && isLidUser(destinationJid) && additionalAttributes?.peer_recipient_pn
            ...
            if (tcTokenBuffer?.length && isTcTokenExpired(existingTokenEntry?.timestamp)) {
                logger.debug({ jid: destinationJid, tcTokenJid: activeTcTokenJid, timestamp: existingTokenEntry?.timestamp }, 'tctoken expired, will re-fetch');
            ...
            if (!tcTokenBuffer?.length && is1on1Send) {
                try {
                    logger.debug({ jid: destinationJid, tcTokenJid, candidates: tcTokenCandidateJids, timeoutMs: preSendTcTokenTimeoutMs }, 'tctoken missing, requesting from server (bounded wait)');
                    const fetchJids = [
                        destinationJid,
                        ...(peerRecipientPn && isPnUser(peerRecipientPn) ? [peerRecipientPn] : [])
                    ].filter((candidate, index, candidates) => candidates.indexOf(candidate) === index);
            ...
            if (tcTokenBuffer?.length) {
                ...
                if (csTokenContent?.length) {
                    ;
                    stanza.content.push(...csTokenContent);
            ...
            if (shouldIncludeBizBinaryNode(innerMessage)) {
                ;
                stanza.content.push(getBizBinaryNode(innerMessage));
    ...
    const getPrivacyTokens = async (jids, timestamp, timeoutMs) => {
    ...
    const ensurePrivacyTokens = async (jids, timeoutMs) => {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (typeof content === 'object' && 'react' in content && content.react?.key) {
                ...
                if (reactKey.participant === '') {
                    delete reactKey.participant;
                    logger?.warn({ reactKey }, 'cleared empty reaction participant');
                }
                if (reactKey.remoteJid && reactKey.remoteJid !== jid) {
                    logger?.warn({ jid, reactRemoteJid: reactKey.remoteJid }, 'reaction target jid mismatch; using react key remoteJid');
