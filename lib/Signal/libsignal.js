...
export function makeLibSignalRepository(auth, logger, pnToLIDFunc) {
    ...
    const runIsolated = (exec, key) => parsedKeys.isolatedTransaction ? parsedKeys.isolatedTransaction(exec) : parsedKeys.transaction(exec, key);
    ...
    const repository = {
        ...
        async decryptMessage({ jid, type, ciphertext }) {
            ...
            return sessionMutex.mutex(sessionKey, () => runIsolated(async () => {
                if (type === 'pkmsg') {
                    const identityKey = extractIdentityFromPkmsg(ciphertext);
                    if (identityKey) {
                        const identityChanged = await storage.saveIdentity(sessionKey, identityKey);
                        if (identityChanged) {
                            logger.info({ jid, addr: sessionKey }, 'identity key changed or new contact, session will be re-established');
                        }
                    }
                }
                switch (type) {
        ...
        async deleteSessionForJid(jid) {
            ...
            await sessionMutex.mutex(sessionKey, () => runIsolated(async () => {
        ...
        async deleteSession(jids) {
            ...
            const sessionKeys = await Promise.all(jids.map(async (jid) => {
                const sessionKey = await resolveSessionKey(jid);
                sessionUpdates[sessionKey] = null;
                return sessionKey;
            }));
            return withSessionLocks(sessionKeys, () => runIsolated(async () => {
        ...
        async migrateSession(fromJid, toJid) {
            ...
            return withSessionLocks(mutexKeys, () => runIsolated(async () => {
