...
import { isTcTokenExpired, resolveIssuanceJid, resolveTcTokenJid, storeTcTokensFromIqResult } from '../Utils/tc-token-utils.js';
...
export const makeMessagesRecvSocket = (config) => {
    ...
    const { userDevicesCache, devicesMutex, ev, authState, ws, messageMutex, notificationMutex, receiptMutex, signalRepository, query, upsertMessage, resyncAppState, onUnexpectedError, assertSessions, sendNode, relayMessage, sendReceipt, sendMessage, uploadPreKeys, sendPeerDataOperationMessage, messageRetryManager, registerSocketEndHandler, issuePrivacyTokens, fetchAccountReachoutTimelock, fetchNewChatMessageCap, getPrivacyTokens } = sock;
    ...
    const dispatchReceiptAfterDecryption = (jid, participant, ids, type, key) => {
        if (type === 'sender' && !participant) {
            logger.warn({ key }, 'skipping sender receipt: missing participant');
            return;
        }
        void sendReceipt(jid, participant ?? undefined, ids, type).catch(err => logger.error({ err, key, receiptType: type }, 'failed to send delivery receipt'));
    };
    ...
    const logMexRestrictionDiagnostics = async (from, msgId) => {
        ...
        const diagnostics = {
            ...
            error_data: {
                ...
                reachout,
                capping,
                reachoutError: reachoutResult.status === 'rejected'
                    ? reachoutResult.reason?.message || String(reachoutResult.reason)
                    : undefined,
                cappingError: capResult.status === 'rejected'
